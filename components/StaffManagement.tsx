...
// Add Star to the lucide-react import to resolve "Cannot find name 'Star'" error
...
export const StaffManagement: React.FC<StaffManagementProps> = ({ staffList, onRemoveStaff, onUpdateStaff, onAddStaff, selectedShift, records }) => {
  ...
  const getAbsenceBadgeColor = (reason: string) => {
    switch (reason) {
      ...
      case AbsenceReason.DIA_EXAMEN:
      case AbsenceReason.DIA_PREEXAMEN:
      case AbsenceReason.CASAMIENTO:
      case AbsenceReason.NACIMIENTO:
      case AbsenceReason.DUELO:
        return 'bg-blue-50 text-blue-700 border-blue-200';
      case AbsenceReason.LICENCIA_MEDICA:
      case AbsenceReason.ARTICULO_95:
        return 'bg-teal-50 text-teal-700 border-teal-200';
      case AbsenceReason.DIA_FEMENINO:
      case AbsenceReason.MATERNIDAD:
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm flex items-center gap-4 group hover:shadow-md transition-all">
          ...
          <div>
            ...
            <h3 className="text-2xl font-black text-slate-800 leading-none mt-1">{stats.total}</h3>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm flex items-center gap-4 group hover:shadow-md transition-all border-l-4 border-l-emerald-500">
          ...
          <div>
            ...
            <h3 className="text-2xl font-black text-emerald-600 leading-none mt-1">{stats.operativos}</h3>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm flex items-center gap-4 group hover:shadow-md transition-all border-l-4 border-l-amber-500">
          ...
          <div>
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Reserva</p>
            <h3 className="text-2xl font-black text-amber-600 leading-none mt-1">{stats.reserva}</h3>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm flex items-center gap-4 group hover:shadow-md transition-all border-l-4 border-l-red-500">
          ...
          <div>
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Faltas</p>
            <h3 className="text-2xl font-black text-red-600 leading-none mt-1">{stats.faltas}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-2">
        <div>
          <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight italic">Padrón {selectedShift}</h2>
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Total {displayedStaff.length} colaboradores filtrados</p>
        </div>
                ...
                <tr key={s.id} className="h-20 hover:bg-slate-50/80 transition-colors group">
                  <td className="pl-8">
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm shadow-sm transition-transform group-hover:scale-110 ${s.status === StaffStatus.ABSENT ? 'bg-red-50 text-red-600' : s.status === StaffStatus.RESERVA ? 'bg-amber-50 text-amber-600' : 'bg-indigo-50 text-indigo-600'}`}>
                        {s.name.charAt(0)}
                      </div>
                      <div>
                        <p className="font-black text-slate-800 text-xs uppercase leading-tight">{s.name}</p>
                        <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">LEGAJO: {s.id}</p>
                      </div>
                    </div>
                  </td>
                  <td className="text-center">
                    <span className={`text-[10px] font-black uppercase tracking-tighter ${s.gender === 'FEMENINO' ? 'text-pink-500' : 'text-indigo-400'}`}>
                        {s.gender === 'FEMENINO' ? 'Mujer' : 'Hombre'}
                    </span>
                  </td>
                  <td className="text-center">
                    <span className="text-[8px] font-black px-2 py-1 bg-slate-100 rounded-lg text-slate-500 uppercase tracking-tighter inline-block min-w-[80px] leading-none">{s.role || 'SIN ROL'}</span>
                  </td>
                  <td className="text-center">
                  ...
                  <td className="text-center">
                    <div className="flex flex-col items-center gap-1">
                      {s.status === StaffStatus.ABSENT ? (
                        <div className={`flex flex-col p-1.5 rounded-xl border-2 shadow-sm ${getAbsenceBadgeColor(s.address || '')}`}>
                            <div className="flex items-center gap-1.5 text-[9px] font-black uppercase">
                                <AlertCircle size={12} className="shrink-0" />
                                <span>{s.address || 'FALTA'}</span>
                            </div>
                        </div>
                      ) : (
                        <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase border-2 ${s.status === StaffStatus.RESERVA ? 'text-amber-600 bg-amber-50 border-amber-100' : 'text-emerald-600 bg-emerald-50 border-emerald-100'}`}>
                          {s.status}
                        </span>
                      )}
                    </div>
                  ...
                  <td className="pr-8 text-right">
                    <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                        ...
                        className="p-2.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all shadow-sm bg-white border border-slate-100"
                      >
                        <Edit3 size={16} />
                      </button>
                      <button 
