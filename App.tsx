                ...
                <div className="h-full flex flex-col">
                    <div className={`px-6 py-2 border-b flex items-center justify-between shrink-0 shadow-sm z-40 transition-colors duration-500 ${activeTab === 'adn' ? 'bg-amber-100/50 border-amber-200' : 'bg-white border-slate-200'}`}>
                        ...
                        {shiftFilter !== 'TODOS' && shiftManagers[shiftFilter] && (
                            <ShiftManagersTop shift={shiftFilter} data={shiftManagers[shiftFilter]} staffList={staffList} onOpenPicker={(f, r) => setPickerState({ type: (activeTab === 'adn' ? 'adn-managers' : 'managers'), targetId: shiftFilter, field: f as string, role: r })} onUpdateStaff={handleUpdateStaff} />
                        )}
                    </div>
                    <div className="flex-1 p-4 overflow-hidden">
                        <div className={`h-full bg-white rounded-2xl shadow-xl border overflow-hidden flex flex-col transition-colors duration-500 ${activeTab === 'adn' ? 'border-amber-400 ring-4 ring-amber-500/10' : 'border-slate-200'}`}>
                            {subTab === 'TRANSFERENCIA' ? (
                                <TransferTable isMasterMode={activeTab === 'adn'} data={transferRecords.filter(tr => shiftFilter === 'TODOS' || tr.shift === shiftFilter)} onUpdateRow={(id, f, v) => setTransferRecords(prev => prev.map(tr => tr.id === id ? {...tr, [f]: v} : tr))} onOpenPicker={(id, field, role, uIdx) => setPickerState({ type: (activeTab === 'adn' ? 'adn-transfer' : 'transfer'), targetId: id, field, role, unitIdx: uIdx })} onUpdateStaff={handleUpdateStaff} />
                            ) : (
                                <ReportTable isMasterMode={activeTab === 'adn'} data={records.filter(r => (shiftFilter === 'TODOS' || r.shift === shiftFilter) && (subTab === 'GENERAL' ? (r.category !== 'REPASO_LATERAL') : (r.category === 'REPASO_LATERAL')))} onUpdateRecord={(id, f, v) => setRecords(prev => prev.map(r => r.id === id ? {...r, [f]: v} : r))} onDeleteRecord={id => setRecords(prev => prev.filter(r => r.id !== id))} onOpenPicker={(id, field, role) => setPickerState({ type: (activeTab === 'adn' ? 'adn-route' : 'route'), targetId: id, field, role })} onUpdateStaff={handleUpdateStaff} activeShiftLabel={activeTab === 'adn' ? `TURNO ${shiftFilter} (ADN)` : `TURNO ${shiftFilter}`} selectedDate={selectedDate} />
                            )}
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-4 bg-[#f8fafc]">
                    {staffList.filter(s => s.name.toLowerCase().includes(pickerSearch.toLowerCase()) || s.id.includes(pickerSearch)).map(s => (
                        <div key={s.id} onClick={() => s.status !== StaffStatus.ABSENT && handlePickerSelection(s)} className={`p-6 rounded-[2rem] border-2 bg-white shadow-sm flex items-center justify-between cursor-pointer hover:border-indigo-400 transition-all ${s.status === StaffStatus.ABSENT ? 'opacity-50 grayscale' : ''}`}>
